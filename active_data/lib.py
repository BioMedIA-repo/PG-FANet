...
def transforms_for_noise(inputs_u2, std,device):
    gaussian = np.random.normal(0, std,
                                (inputs_u2.shape[0], inputs_u2.shape[1], inputs_u2.shape[-1], inputs_u2.shape[-2]))
...
def transforms_for_rot(ema_inputs):
    ...
    output = torch.zeros_like(ema_inputs).to(ema_inputs.get_device())
    for idx in range(ema_inputs.shape[0]):
        if flip_mask[idx] == 1:
            output[idx] = torch.flip(ema_inputs[idx], [1])
            output[idx] = torch.rot90(output[idx], int(rot_mask[idx]), dims=[1, 2])
        else:
            output[idx] = torch.rot90(ema_inputs[idx], int(rot_mask[idx]), dims=[1, 2])

    return output, rot_mask, flip_mask


# def transforms_back_rot(ema_output, rot_mask, flip_mask):
#     output = torch.zeros_like(ema_output).to(ema_inputs.get_device())
#     for idx in range(ema_output.shape[0]):
#
#         output[idx] = torch.rot90(ema_output[idx], int(rot_mask[idx]), dims=[2, 1])
#
#         if flip_mask[idx] == 1:
#             output[idx] = torch.flip(output[idx], [1])
#
#
#     return output
...
def crop_output_for_shift(ema_inputs, shift_mask, scale_mask):
    ...
    for idx in range(ema_inputs.shape[0]):
        ...
        if shift_mask[idx] == 0:
            ema_outputs[idx, :, 0:224 - scale_mask[idx], 0:224 - scale_mask[idx]] = torch.from_numpy(
                new_img[scale_mask[idx]:, scale_mask[idx]:, :].transpose((2, 0, 1)))
        elif shift_mask[idx] == 1:
            ema_outputs[idx, :, 0:224 - scale_mask[idx], 0:224 - scale_mask[idx]] = torch.from_numpy(
                new_img[:-scale_mask[idx], scale_mask[idx]:, :].transpose((2, 0, 1)))
        elif shift_mask[idx] == 2:
